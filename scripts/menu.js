...
var ___aa = document.location.href.split('/');

var CONTEXT_ROOT = '.';
try
{
	CONTEXT_ROOT = '/' + ___aa[3];
}
catch (e)
{
	alert('Cannot find context root! (from menu.js)');
}
...
if (typeof menu!="undefined"){
	for(i=0; i<Math.max(titles.length, submenus.length); i++){
		...
		arrows[i].src = CONTEXT_ROOT + "/images/expanded.gif";
...
if (typeof menu!="undefined" && typeof titles!="undefined" && typeof submenus!="undefined"){
	for(i=0; i<Math.max(titles.length, submenus.length); i++){
		titles[i].className="titlehidden";
		arrows[i].src = CONTEXT_ROOT + "/images/collapsed.gif";
...
function restore() {
    if(getcookie("__menu") != null) {
        var hidden = getcookie("__menu").split(",");
        for(var i =0; hidden != null && i < hidden.length; i++) {
			if (isNumeric(hidden[i]))
			{
                titles[hidden[i]].className = "titlehidden";
                submenus[hidden[i]].style.height = "0px";
                submenus[hidden[i]].style.display = "none";
                arrows[hidden[i]].src = CONTEXT_ROOT + "/images/collapsed.gif";
			}
        }
    }
}
...
function hidemenu(sm) {
    ...
    if(parseInt(submenus[sm].style.height) <= nr) {
        ...
        arrows[sm].src = CONTEXT_ROOT + "/images/collapsed.gif";
...
function showmenu(sm) {
    ...
    if(parseInt(submenus[sm].style.height) > (parseInt(heights[sm])-nr)) {
        ...
        arrows[sm].src = CONTEXT_ROOT + "/images/expanded.gif";
    }
...
function store() {
    ...
    putcookie("__menu", hidden.join(","), 30);
}
